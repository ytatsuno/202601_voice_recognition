...
音声データのCNN（畳み込みニューラルネットワーク）学習を行ったモデルの作成手順と、Webアプリでの推論実装（マイク入力→MFCC→predict）までをまとめています。
...
- 短い単語（キーワード）を音声で認識し、Webアプリを操作できる点は、TensorFlow.js を使うことで実現できる新しいユーザビリティの可能性を感じました。
